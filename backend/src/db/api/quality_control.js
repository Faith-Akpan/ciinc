...
module.exports = class Quality_controlDBApi {
    ...
    static async update(id, data, options) {
        ...
        if (data.inspection_stage !== undefined) updatePayload.inspection_stage = data.inspection_stage;
    ...
    static async findAll(filter, options) {
        ...
        if (filter) {
            if (filter.id) {
                ...
                if (filter.inspection_stage) {
                    where = {
                        ...where,
                        [Op.and]: Utils.ilike(
                            'quality_control',
                            'inspection_stage',
                            filter.inspection_stage,
                        ),
                    };
                }
    ...
    static async findAllAutocomplete(query, limit, offset) {
        ...
        const records = await db.quality_control.findAll({
            attributes: [ 'id', 'inspection_stage' ],
            where,
            limit: limit ? Number(limit) : undefined,
            offset: offset ? Number(offset) : undefined,
            orderBy: [['inspection_stage', 'ASC']],
        });

        return records.map((record) => ({
            id: record.id,
            label: record.inspection_stage,
        }));
    }
